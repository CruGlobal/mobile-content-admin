...
describe('ToolGroupResourceComponent', () => {
  ...
  describe('Successful spys', () => {
    ...
    describe('createOrUpdate()', () => {
      ...
      it('should return and error as suggestionsWeight is empty', () => {
        ...
        expect(comp.errorMessage[0]).toEqual(
          `${tool.tool.name} needs to have a Suggestions Weight larger than 0.`,
        );
        expect(comp.errorMessage[1]).toEqual(
          `secondToolName needs to have a Suggestions Weight larger than 0.`,
        );
      });
  ...
  describe('Unsuccessful spys', () => {
    beforeEach(() => {
      ...
      spyOn(toolGroupServiceStub, 'addOrUpdateTool').and.returnValue(
        Promise.reject('Error Message regarding Data'),
      );
      spyOn(resourceServiceStub, 'getResources').and.returnValue(
        Promise.resolve<Resource[]>(resources),
      );
    ...
    it('should just update one tool', (done) => {
      ...
      setTimeout(() => {
        expect(comp.errorMessage.length).toEqual(2);
        expect(comp.errorMessage[0]).toEqual('Error Message regarding Data');
        expect(comp.errorMessage[1]).toEqual('Error Message regarding Data');
        expect(activeModalStub.close).not.toHaveBeenCalled();
        done();
      });
    });
