...
import { CountryRule, LanguageRule, PraxisRule, RuleTypeEnum, Praxis, PraxisTypeEnum } from '../../models/tool-group';
...
type Item = LanguageBCP47 & countriesType & Praxis | countriesType & Praxis & LanguageBCP47
...
export class ToolGroupRuleReuseableComponent implements OnInit {
  ...
  @Output() selectedItemsEmit = new EventEmitter<(LanguageBCP47|countriesType|Praxis)[]>();
      ...
      case RuleTypeEnum.PRAXIS:
          switch(this.praxisType) {
            case PraxisTypeEnum.CONFIDENCE:
              this.items = Object.entries(this.toolGroupService.praxisConfidentData).map(praxis => {
                return {
                  code: praxis[0],
                  ...praxis[1]
                } as Praxis
              ...
              if (this.rule.confidence) {
                this.selectedItems = this.rule.confidence.map((item) => {
                  return this.items.find(praxis => praxis.code === item.toString());
                });
              }
  ...
  handleDeleteSelectedItem(selecteditem: Item): void {
    this.selectedItems = this.selectedItems.filter((item) => item.code !== selecteditem.code) as unknown as LanguageBCP47[] | countriesType[];
    this.selectedItemsEmit.emit(this.selectedItems);
  }
